...
class ext_tinyissimoYOLO(nn.Module):
    def __init__(self, in_channels=3, initialize = False,**kwargs):
        # 3 channels due to the RGB images
        super(ext_tinyissimoYOLO, self).__init__()
        self.architecture = config.architecture_ext_tinyissimo_YOLO
...
def print_info(model_name = 'ext_tinyissimoYOLO', S = 4, B = 2, C = 20, return_model = False):
    batch_size = 2
    num_channels = 3 # RGB
    ...
    model = ext_tinyissimoYOLO(split_size=S, num_boxes=B, num_classes=C).to(config.DEVICE)
    size_img = 352
