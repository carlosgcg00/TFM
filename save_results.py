...
def save_results(num_epochs, mean_loss, mAP_50, mAP_75, mAP_90, learning_rate, mode = 'train'):
    ...
    with open(os.path.join(config.DRIVE_PATH, f"{config.BACKBONE}/{config.TOTAL_PATH}/results/results_{mode}.txt"), "a+") as file:
...
def plot_results():
    ...
    with open(os.path.join(config.DRIVE_PATH, f'{config.BACKBONE}/{config.TOTAL_PATH}/results/results_train.txt')) as f:
        i = 0
    ...
    # Read the data from the file
    with open(os.path.join(config.DRIVE_PATH, f'{config.BACKBONE}/{config.TOTAL_PATH}/results/results_valid.txt')) as f:
        i = 0
        for line in f.readlines():
            if i!=0:
                # Split the line into its components
    ...
    plt.grid(True)
    plt.savefig(os.path.join(config.DRIVE_PATH,f"{config.BACKBONE}/{config.TOTAL_PATH}/results/epochs_vs_mAP50.png"))
    plt.close()
    ...
    # plt.yscale('log')
    plt.grid(True)
    plt.savefig(os.path.join(config.DRIVE_PATH, f"{config.BACKBONE}/{config.TOTAL_PATH}/results/epochs_vs_mean_loss.png"))
    ...
    plt.ylabel("Mean Average Precision")
    plt.grid(True)
    plt.savefig(os.path.join(config.DRIVE_PATH, f"{config.BACKBONE}/{config.TOTAL_PATH}/results/epochs_vs_mAP50_75_90.png"))
    plt.close() 
    ...
    plt.grid(True)
    plt.savefig(os.path.join(config.DRIVE_PATH,f"{config.BACKBONE}/{config.TOTAL_PATH}/results/epochs_vs_learning_rate.png"))
    plt.close()
    ...
    # Adjust layout
    plt.tight_layout()
    plt.savefig(os.path.join(config.DRIVE_PATH, f"{config.BACKBONE}/{config.TOTAL_PATH}/results/summary_metrics.png"))
    plt.close()
...
def save_log(num_epochs, mean_loss, mean_loss_coord, mean_loss_obj, mean_loss_noobj, mean_loss_class, mode = 'train', eval_mode = 'train'):
    ...
    with open(os.path.join(config.DRIVE_PATH, f"{config.BACKBONE}/{config.TOTAL_PATH}/results/{file_name}"), "a+") as file:
        # Move the cursor to the start of the file to check if it's empty
        file.seek(0)
        # If the file is empty, write a header line
        if not file.readline():
...
def plot_loss(mode = 'train', eval_mode = 'train'):
    ...
    if mode == 'train':
        ...
        file_name = f"results_loss_eval_{eval_mode}.txt"
    with open(os.path.join(config.DRIVE_PATH, f'{config.BACKBONE}/{config.TOTAL_PATH}/results/{file_name}')) as f:
        i = 0
        for line in f.readlines():
            if i!=0:
                # Split the line into its components
    ...
    plt.savefig(os.path.join(config.DRIVE_PATH, f"{config.BACKBONE}/{config.TOTAL_PATH}/results/{file_name}"))
    plt.close()  # Close the plot to ensure it doesn't interfere with the next one
...
def read_last_line(filename=os.path.join(config.DRIVE_PATH,f'{config.BACKBONE}/{config.TOTAL_PATH}/results/results_train.txt')):
