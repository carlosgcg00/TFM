...
Build a YOLO v1 model using a pre-trained VGG16 model from torchvision.
'''

def vgg16(split_size, num_boxes, num_classes, pretrained=True):
    """
    Modifies a pre-trained VGG16 model to build a variant of the YOLO v1 model.
    ...
    :param pretrained: Boolean indicating whether to use a pre-trained VGG16 model
    ...
    # Add custom Adaptive Average Pooling layer
    model.avgpool = nn.AdaptiveAvgPool2d((split_size, split_size))

    # Add custom fully connected (FC) layers
...
def print_info(S=7, B=2, C=1):
    ...
    log_dir = os.path.join(config.ROOT_DIR,'Architecture/vgg16')
...
def test(S=7, B=2, C=20):
    """
    Runs a test case to validate the modified VGG16 model for YOLO v1.
    ...
    x_vgg16 = torch.randn((batch_size, num_channels, 224, 224))

    from torch.utils.tensorboard import SummaryWriter    
    writer = SummaryWriter(os.path.join(config.ROOT_DIR, 'Architecture/vgg16'))
    ...
    print(f"Total parameters in VGG16 model: {total_params}")
